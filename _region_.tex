...
\message{ !name(reynolds.tex) !offset(-39) }
\subsection{The Reynolds Operator And Linearly Reductive Groups}

\begin{definition}[Linearly Reductive Groups]
  Let $G$ be a linear algebraic group.
  We call $G$ \textbf{linearly reductive}, iff for any rational representation $V$ and for any $v \in V^G \setminus \{ 0 \}$ there exists an $f \in \left( V^\ast \right)^G $ such that $f(v) \neq 0$.
\end{definition}
...
\begin{theorem}\label{equiv}
  Let $G$ be a linear reductive group.
...
\begin{proposition}
  ...
  Let $G$ be a linearly reductive group and let $X$ be an affine $G$-variety and let $ R \colon K[X] \twoheadrightarrow K[X]^G $ be the Reynolds operator.
...
We now have a few characterizations of linearly reductive groups, but for a given linear algebraic group, it is still hard to concretely show that it is linearly reductive.
...
\message{ !name(roughdraft.tex) !offset(-340) }
