...
\begin{proof}
  ...
  For $f \in K[X]$ we denote by $W_f$ the unique submodule of $V_f$ such that $V_f = V_f^G \oplus W_f$ as in (b). %yougee
  This map is linear:
...
\begin{proof}
  ...
  We consider the finite-dimensional $G$-submodule $V_g \subseteq K[V]$, and we notice that since $i^\ast$ is $G$-equivariant, $W := i^\ast (V_g)$ is a finite-dimensional $G$-submodule of $K[X]$.  %yougee
