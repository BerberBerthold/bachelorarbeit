...
where $k \in \mathbb{N}$ is the dimension of $W$.
  If we let $G$ act on $ \operatorname{Hom}_K(W,V)$ by \linebreak$\sigma.B :=  w \mapsto \sigma . (B(w))$ and on $\operatorname{Hom}_K(V,W)$ by $ \sigma.A := v \mapsto A(\sigma^{-1}.v) $, we then receive $\Phi$ as an isomorphism of representations. %meaning $\operatorname{Hom}_K(V,W)^G$ corresponds to $(\operatorname{Hom}_K(W,V)^\ast)^G$.
...
In theorem \ref{equiv} we have learned about differnet characterizations of linearly reductive groups, but for a given linear algebraic group, it is still hard to concretely show that it is linearly reductive.
